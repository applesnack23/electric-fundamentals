...
- 실제 자동화 설비에서는 기계식 스위치보다 근접센서, 광센서, 포토센서와 같은 전자식 센서를 훨씬 많이 사용합니다.
- 이러한 센서는 대부분 NPN 또는 PNP 출력 방식을 사용하며, PLC의 입력 방식과 맞게 연결해야 정상적으로 동…
- 이번에는 NPN과 PNP이 차이, Source와 Sink 방식, 그리고 PLC 입출력 모듈의 연결 방법을 알아보겠…
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-01.png)

- NPN 트랜지스터 NPN 트랜지스터는 가장 많이 사용되는 스위칭 소자 중 하나 입니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-02.png)

- PNP 트랜지스터 PNP 트랜지스터 역시 Open Collector 방식으로 많이 사용됩니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-03.png)

- NPN 타입 센서 NPN 센서는 출력이 ON되면 신호선을 0V(GND)로 연결합니다.
- 이를 Sink 출력이라고도 합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-04.png)

- PNP 타입 센서 PNP 센서는 출력이 ON되면 신호선에 +24V를 출력합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-05.png)

- Source 입력 Source 입력은 입력 공통 (Common) 이 +24V에 연결되어 있으며, 입력 단자에 -전…
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-06.png)

- Sink Type Input Sink 입력은 입력 공통(Common)이 0V에 연결되어 있으며, 입력 단자에 전압…
- 따라서 일반적으로 PNP 센서와 함꼐 사용합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-07.png)

- PLC Source 입력 (Common) Source 입력 모듈은 Common 단자를 +24V에 연결하여 사용합니…
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-08.png)

- PLC Sink 입력 (Common) Sink 입력 모듈은 Common 단자를 0V에 연결하여 사용합니다.
- 이 경우 NPN 센서를 연결해야 정상적으로 동작합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-09.png)

- PLC Source 출력 (Common) Source 출력 모듈은 출력시 +24V 를 공급하는 방식입니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-10.png)

- PLC Sink 출력 (Common) Sink 출력 모듈은 출력시 0V를 연결하는 방식입니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-11.png)

- 릴레이 출력 모듈 릴레이 출력 모듈은 반도체 출력과 달리 내부에 실제 릴레이 접점이 있습니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-12.png)
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-13.png)
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-14.png)

- 투과형 센서 투과형 센서는 발광부와 수광부가 서로 마주보는 구조입니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-15.png)

- 반사형 센서 반사형 센서는 발광부와 수광부가 하나의 센서 안에 있습니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-16.png)
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-17.png)
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-18.png)

- 입력 모듈 사양 입력 모듈은 스위치나 센서의 신호를 PLC로 전달하는 역할을 합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-19.png)

- 입력 모듈 외부 결선도 입력 모듈은 Common 단자와 센서를 올바르게 연결해야 정상적으로 동작합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-20.png)

- 센서 연결 센서는 전원선과 신호선을 각각 연결하여 PLC 입력으로 전달합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-21.png)

- 입력 연결 센서의 출력 신호는 PLC 입력 단자로 연결되며, PLC는 이를 ON/OFF 신호로 인식합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-22.png)

- 출력 모듈 출력 모듈은 PLC 내부의 제어 신호를 외부 장치로 전달합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-23.png)

- 출력모듈 외부 결선도 출력 모듈의 Common 단자를 올바르게 연결한 후 부하를 연결합니다.
...
![bg right:42% contain](../assets/11-npn-pnp-sensors-plc-io-24.png)

- 출력 연결 PLC 출력이 ON되면 부하에 전원이 공급되어 장치가 동작합니다.
